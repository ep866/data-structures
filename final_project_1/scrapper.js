...
var parser = {
    request: function(url, file) {
        request(url, function(error, response, body){
            if( !error && response.statusCode == 200) {
                ...
                fs.writeFileSync('scrapped_data/' + file + '.txt', body);
    ...
    cacheFiles: function() {
        ...
        async.eachSeries(meetings, function(url, callback) {
            ...
            if( fs.existsSync('scrapped_data/' + meetingName + '.txt') ) {
    ...
    extract: function(data) {
        ...
        fs.writeFileSync('processed_data/all_meetings.json', JSON.stringify(meetingsByAddress, null, 2) , 'utf-8');
    ...
    augmentData: function() {
        ...
        parser.read('processed_data/all_meetings.json', function(meetings) {
            ...
            if( uniqueLocations < apiLimit ) {
                ...
                setTimeout(callback, 2000);
                    ...
                    fs.writeFileSync('processed_data/augmented_meetings.json', JSON.stringify(augmentedMeetings, null, 2) , 'utf-8');
                });
...
parser.entities = {
    ...
    meeting: function() {
        ...
        $("#meetings_tbody tr").each(function(i, e){
            ...
            // load details file
            parser.read('scrapped_data/' + meetingName + '.txt', function(meetingDetails){
...
parser.read("scrapped_data/meetings.txt", function(html) {
